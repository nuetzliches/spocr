...
public sealed class FunctionSnapshotCollector
{
    ...
    public async Task CollectAsync(SchemaSnapshot snapshot, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            foreach (var r in fnRows)
            {
                ...
                // Heuristik FOR JSON Erkennung (nur wenn Definition verfügbar)
                bool? returnsJson = null;
                bool? returnsJsonArray = null;
                string jsonRoot = null;
                if (!string.IsNullOrWhiteSpace(definition))
                {
                    // Vereinfachtes Regex: erkennt 'FOR JSON (PATH|AUTO)?(,\s*ROOT\s*\('Alias'\))?(,\s*WITHOUT_ARRAY_WRAPPER)?'
                    try
                    {
                        var pattern = @"FOR\s+JSON\s+(PATH|AUTO)?(?<rest>[^;]*)"; // grob bis Statementende
                        ...
                        if (match.Success)
                        {
                            ...
                            if (!string.IsNullOrEmpty(rest))
                            {
                                ...
                                {
                                    returnsJsonArray = false; // Einzelobjekt
                                }
                                else
                                {
                                    returnsJsonArray = true; // Default: Array
                                }
                                ...
                                {
                                    jsonRoot = rootMatch.Groups[1].Value;
                                }
                            }
                        }
                    }
                    catch { /* Fallback: ignorieren */ }
                }
                ...
                // JSON Columns für skalare Funktionen extrahieren (Heuristik) – nur wenn Definition vorhanden, nicht verschlüsselt, nicht TVF
                if (!isTableValued && returnsJson == true && !encrypted && !string.IsNullOrWhiteSpace(definition))
                {
                    try
                    {
                        ...
                        if (inferred.Count > 0)
                        {
                            ...
                            _console.Verbose($"[fn-json-cols] {schema}.{name} count={finalList.Count}");
                        }
                        else
                        {
                    ...
                    {
                        _console.Verbose($"[fn-json-cols-error] {schema}.{name} {jex.Message}");
                    }
                }
            ...
            foreach (var p in paramRows)
            {
                ...
                if (!fn.IsTableValued && (string.IsNullOrWhiteSpace(name)))
                {
    ...
    private static List<SnapshotFunctionColumn> InferJsonColumns(string definition, string schema, string name)
    {
        ...
        try
        {
            // Normalisieren für Regex (Zeilenumbrüche behalten wegen Singleline)
            var def = definition;
            // Wir suchen das letzte "FOR JSON" und davor ein SELECT ... FROM ... Muster
            // Regex: SELECT <selectList> FROM ... FOR JSON
            var match = Regex.Match(def, @"SELECT\s+(?<select>.+?)\s+FROM\s+.+?FOR\s+JSON", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            if (!match.Success)
            {
            ...
            // Zerlegen der SELECT Liste an Top-Level-Kommas
            var segments = SplitTopLevel(selectList);
            foreach (var rawSegment in segments)
            {
                var seg = rawSegment.Trim();
                if (string.IsNullOrWhiteSpace(seg)) continue;
                // Alias-Erkennung: Formate
                // 1) alias = expression
                // 2) expression AS alias
                // 3) einfacher Ausdruck (wir versuchen letzten Identifier nach Punkt)
                string alias = null;
                string expression = seg;
                var assignMatch = Regex.Match(seg, @"^(?<alias>\[[^]]+\]|[A-Za-z0-9_]+)\s*=\s*(?<expr>.+)$", RegexOptions.IgnoreCase | RegexOptions.Singleline);
                if (assignMatch.Success)
                {
                    alias = StripBrackets(assignMatch.Groups["alias"].Value);
                    expression = assignMatch.Groups["expr"].Value;
                }
                else
                {
                    var asMatch = Regex.Match(seg, @"^(?<expr>.+?)\s+AS\s+(?<alias>\[[^]]+\]|[A-Za-z0-9_]+)$", RegexOptions.IgnoreCase | RegexOptions.Singleline);
                    if (asMatch.Success)
                    {
                        alias = StripBrackets(asMatch.Groups["alias"].Value);
                        expression = asMatch.Groups["expr"].Value;
                    }
                }
                if (alias == null)
                {
                    // Fallback: Versuche einfachen Spaltennamen zu extrahieren
                    // Entferne CAST()/CONVERT(), Klammern am Anfang
                    var cleaned = Regex.Replace(expression, @"^(CAST|CONVERT)\s*\(", "", RegexOptions.IgnoreCase);
                    cleaned = cleaned.Trim();
                    // Wenn Ausdruck wie t.Column oder [t].[Column]
                ...
                if (string.IsNullOrWhiteSpace(alias)) alias = "col" + result.Count.ToString();

                // Typ-Heuristik
                string sqlType = null;
                int maxLen = 0;
                bool isNullable = true; // JSON Properties meist optional

                // Nested JSON Subselect?
                if (Regex.IsMatch(expression, @"SELECT.+FOR\s+JSON", RegexOptions.IgnoreCase | RegexOptions.Singleline))
                {
                    sqlType = "json"; // Kennzeichnen verschachteltes JSON Objekt/Array
                    maxLen = -1;
                }
                else
                {
                    ...
                    if (castType.Success)
                    {
                        sqlType = castType.Groups["type"].Value;
                    }
                    else
                    {
                        ...
                        if (convType.Success)
                        {
                            sqlType = convType.Groups["type"].Value;
                        }
                    }
                    if (sqlType == null)
                    {
                        if (Regex.IsMatch(expression, @"JSON_QUERY|JSON_VALUE", RegexOptions.IgnoreCase))
                        {
                            sqlType = "nvarchar(max)";
                        }
                        else
                        {
                            // Default für JSON Scalar Properties
                            sqlType = "nvarchar(max)";
                        }
                    }
                ...
                result.Add(new SnapshotFunctionColumn
                {
                    Name = alias,
                    SqlTypeName = sqlType,
                    IsNullable = isNullable,
        ...
        catch { /* Ignorieren – leere Liste als Fallback */ }
        return result;
    }
