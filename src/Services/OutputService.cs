...
#pragma warning restore CS0618
    ...
    public async Task WriteAsync(string targetFileName, SourceText sourceText, bool isDryRun)
    {
        ...
        if (!outputFileText.Contains(headerMarker))
        {
            ...
            {
                var timestamp = System.DateTime.UtcNow.ToString("u").Replace(' ', ' ');
                header += $"/// <remarks>Generated at {timestamp}</remarks>\r\n";
            }
        ...
        if (File.Exists(targetFileName))
        {
            ...
            // Normalize by stripping timestamps from remarks for comparison stability
            string StripTimestamp(string text)
                => System.Text.RegularExpressions.Regex.Replace(text,
                    @"/// <remarks>Generated at .*?</remarks>\r?\n?", "", System.Text.RegularExpressions.RegexOptions.Multiline);

            var normalizedExisting = StripTimestamp(existingFileText);
            var normalizedNew = StripTimestamp(outputFileText);
            var upToDate = string.Equals(normalizedExisting, normalizedNew, System.StringComparison.Ordinal);
