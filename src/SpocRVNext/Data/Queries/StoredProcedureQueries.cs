...
internal static class StoredProcedureQueries
{
    ...
    public static async Task<StoredProcedureDefinition?> StoredProcedureDefinitionAsync(this DbContext context, string schemaName, string name, CancellationToken cancellationToken)
    {
        ...
        const string queryString = @"SELECT s.name AS schema_name, o.name, o.object_id AS id, m.definition
                               FROM sys.objects AS o
                               INNER JOIN sys.schemas AS s ON s.schema_id = o.schema_id
                               INNER JOIN sys.sql_modules AS m ON m.object_id = o.object_id
                               WHERE o.object_id = @objectId";
        return await context.SingleAsync<StoredProcedureDefinition>(queryString, parameters, cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        var storedProcedure = await context.ObjectAsync(schemaName, name, cancellationToken).ConfigureAwait(false);
        if (storedProcedure == null)
        {
            return null;
        }

        var parameters = new List<SqlParameter> { new("@objectId", storedProcedure.Id) };
        ...
        return await context.ListAsync<StoredProcedureOutput>(queryString, parameters, cancellationToken).ConfigureAwait(false);
    }
    ...
    public static async Task<List<StoredProcedureInput>?> StoredProcedureInputListAsync(this DbContext context, string schemaName, string name, CancellationToken cancellationToken)
    {
        ...
        var parameters = new List<SqlParameter> { new("@objectId", storedProcedure.Id) };
        const string queryString = @"SELECT p.name,
        ...
        return await context.ListAsync<StoredProcedureInput>(queryString, parameters, cancellationToken).ConfigureAwait(false);
    }
    ...
    public static async Task<string?> StoredProcedureContentAsync(this DbContext context, string schemaName, string name, CancellationToken cancellationToken)
    {
        ...
        var parameters = new List<SqlParameter> { new("@objectId", storedProcedure.Id) };
        const string queryString = @"SELECT definition FROM sys.sql_modules WHERE object_id = @objectId;";
        var content = await context.SingleAsync<StoredProcedureContent>(queryString, parameters, cancellationToken).ConfigureAwait(false);
    ...
    public static Task<DbObject?> ObjectAsync(this DbContext context, string schemaName, string name, CancellationToken cancellationToken)
    {
        ...
        const string queryString = @"SELECT o.object_id
                                FROM sys.objects AS o
                                INNER JOIN sys.schemas AS s ON s.schema_id = o.schema_id
