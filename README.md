...
- Stable Diffs: Auto-generated `<remarks>` timestamps are ignored during change detection to reduce churn
...
#### Volatile Metadata Removal
Earlier versions included a `GeneratedUtc` timestamp inside each snapshot file causing noisy diffs. This value has been moved to a non‑versioned cache file under `.spocr/cache/<fingerprint>.meta.json` (gitignored). The snapshot JSON is now stable unless real schema content changes. If you relied on `GeneratedUtc`, read the meta file; do not reintroduce time fields into the persisted snapshot.
...
### Timestamp Normalization in Generated Files

SpocR injects an XML header into generated files:

```csharp
/// <summary>Auto-generated by SpocR. DO NOT EDIT. Changes will be overwritten on rebuild.</summary>
/// <remarks>Generated at 2025-10-09 12:34:56Z</remarks>
```

To avoid noisy diffs, the internal comparison logic strips the `<remarks>Generated at ...</remarks>` line before deciding whether a file is `Modified` or `UpToDate`. This means:

1. Rebuilds will not mark files as modified solely due to timestamp changes.
2. External diff tools still show the line if you compare raw text; suppress it manually if needed.
3. A future configuration flag (`project.output.suppressTimestamps`) may allow removal of the timestamp line entirely.

Rationale: Generated-at timestamps aid provenance during manual inspection but should not cause spurious churn in CI or PR reviews.

If you depend on the raw timestamp for downstream tooling, parse it before normalization or disable future suppression once the flag is available.
