<#
.SYNOPSIS
  Minimal smoke test for the sample REST API.
.DESCRIPTION
  Only exercises endpoints defined in Program.cs:
    1. Build
    2. Start
    3. GET /
    4. GET /api/users
    5. POST /api/users
    6. Stop
  Fails fast (first non-success stops with exit code 1). Keep it lean for CI.
#>
...
$psi.FileName = 'dotnet'
if ($Framework) {
  $psi.Arguments = "run --project $Project --framework $Framework --urls=$BaseUrl"
} else {
  $psi.Arguments = "run --project $Project --urls=$BaseUrl"
}
...
if ($users.StatusCode -ne 200){
  Write-Host ($users.Content | Out-String) -ForegroundColor DarkGray
  Fail "/api/users Status=$($users.StatusCode)"
} else { Ok "/api/users ok (len=$([Math]::Min($users.Content.Length,200)))" }
...
if ($created.StatusCode -ne 201){
  Write-Host ($created.Content | Out-String) -ForegroundColor DarkGray
  Fail "POST /api/users Status=$($created.StatusCode)"
} else { Ok "POST /api/users ok" }
